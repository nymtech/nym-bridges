...
fn wireguard_test_peer(
    ...
    let static_private: x25519::StaticSecret = <[u8; 32]>::from_hex(static_private).unwrap().into();
    let peer_static_public: x25519::PublicKey =
        <[u8; 32]>::from_hex(peer_static_public).unwrap().into();

    let peer = Tunn::new(static_private, peer_static_public, None, None, 100, None).unwrap();

    let peer: Arc<Box<Tunn>> = Arc::new(Box::new(peer));
...

    {
        ...
        thread::spawn(move || {
            loop {
                ...
                match peer.as_mut().decapsulate(None, &recv_buf[..n], &mut send_buf) {
...
fn get_client_keys() -> ClientConfig {
    ClientConfig {
        // the responders private key
        private_key: BASE64_STANDARD.decode("QPKlgURzcyxu2Hh0GHwP2RB/Tp24Z33ejT6pnba99lQ=").unwrap(),
        // the responders public key
        public_key: BASE64_STANDARD.decode("9LVtZwHPXD0KMdYKRPmyiQhE+/hTcPuT8eCmLo5PsXw=").unwrap(),
    }
}

fn get_wg_server() -> WGServer {
    WGServer {
        // the initiators pubic key
        public_key: BASE64_STANDARD.decode("bZPrbacCwTLOsh+MTUlI7rbd2Wz4kjvYHg2LHkBuZFU=").unwrap(),
        // the initiators public IP
        public_ip: "10.0.0.1".into(),
        // public_ip: "127.0.0.1:51821".into(), // if it means the public address
    }
}
